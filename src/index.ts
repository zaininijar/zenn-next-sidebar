...
export function getSidebarRoutes(baseDir: string = process.cwd()): Route[] {
    const routeDir = detectNextJsRouteDir(baseDir);
    return walkRoutes(routeDir, routeDir);
}

function detectNextJsRouteDir(baseDir: string): string {
...
function walkRoutes(currentDir: string, baseDir: string): Route[] {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            if (relativePath === '/page') relativePath = '/';
